...
- (void)commonInit
{
    ...
    
    {
        ...
        _circleView.backgroundColor = [UIColor colorWithHexString:@"DFE9E4"];
...
//        _labNumber.backgroundColor = [UIColor greenColor];
...
- (void)layoutSubviews
{
    ...
    if (_manager.settings.calendarTextAlineCenter) {
    ...
    _labNumber.frame = CGRectMake(0, self.bounds.size.height - 20, self.bounds.size.width, 20);
...
//    _circleView.layer.cornerRadius = sizeCircle / 2.; //cancel layer
    
    _dotView.frame = CGRectMake(0, 0, self.bounds.size.width-2, self.bounds.size.height-2);
    _dotView.center = CGPointMake(self.frame.size.width / 2., (self.frame.size.height / 2.) +sizeDot * 2.5);
//    _dotView.layer.cornerRadius = sizeDot / 2.; //cancel layer
}
